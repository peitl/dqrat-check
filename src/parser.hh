...
namespace DQRATCheck {
	...
	inline void skip_space(const string& str, size_t& idx) {
	...
	class Parser {
		...
		std::map<string, int32_t> qcir_cls_var_conversion_map;
		std::map<string, int32_t> qcir_trm_var_conversion_map;
		int32_t nr_vars;
		...
		bool readQDIMACS(DQBF& dqbf, std::istream& ifs = std::cin);
		void writeQDIMACS();
